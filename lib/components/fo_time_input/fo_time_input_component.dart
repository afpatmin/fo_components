...
class FoTimeInputComponent implements ControlValueAccessor<String>
{
  ...
  void writeValue(String obj)
  {
    ...
    {
      RegExp regexp = new RegExp(r"([01]\d|2[0-3]):?([0-5]\d)");

      if (regexp.allMatches(obj).length != 1)
        throw new FormatException("Value must be formatted as HH:MM");
  ...
  void set selectedHour(String hour)
  {
    _hour = hour;
    if (_onChange != null) _onChange("$_hour:$_minute");
  }

  void set selectedMinute(String minute)
  {
    _minute = minute;
    if (_onChange != null) _onChange("$_hour:$_minute");
  }
